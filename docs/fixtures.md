# The sandbox, the seed and the fixtures

The page runs on one seeded sandbox project. `tools/seed.py` writes it, `tools/capture.py` snapshots
it into `fixtures/`, and the fixtures are the reference for what a field looks like, never a data
source: there is no mock.
